...
export async function saveAudit(audit) {
  ...
  // The one-off "site photo" (e.g. a clean water point with no issues to
  // report) isn't a checklist item — same upload, its own field.
  let sitePhotoUrl = null;
  if (audit.sitePhoto) {
    const { buffer, mimeType } = dataUrlToBuffer(audit.sitePhoto);
    const blob = await put(`photos/${audit.id}/${audit.id}_site.jpg`, buffer, {
      access: "public",
      contentType: mimeType,
      addRandomSuffix: false,
      allowOverwrite: true,
    });
    sitePhotoUrl = blob.url;
    photosUploaded++;
  }

  const record = { ...audit, checklist, sitePhoto: sitePhotoUrl, syncedAt: new Date().toISOString() };
  ...
  // Index-aligned with the `checklist` array the client sent (see
  // auditRecord() in lib/engine.js) so the client can swap each item's
  // heavy base64 photos for these tiny URLs once they're safely stored
  // here — otherwise the base64 payload never leaves localStorage even
  // after a successful sync.
  const photosByItem = checklist.map((c) => c.photos.map((p) => p.url));
  return { syncedAt: record.syncedAt, findingsWritten: issues.length, photosUploaded, photosByItem, sitePhotoUrl };
}
...
/**
 * Delete every synced audit and every photo — the whole "audits/" and
 * "photos/" prefixes, wiped clean. For starting a fresh round of testing
 * (or a fresh mission) without any old test data lingering in the admin
 * page, the Google Sheet export, or the daily Drive-photo routine.
 * There's no undo: once a blob is deleted here it's gone from Blob
 * storage for good (any copy already uploaded to Google Drive by the
 * daily routine is untouched, since that's a separate copy).
 */
export async function deleteAll() {
  let deleted = 0;
  for (const prefix of ["audits/", "photos/"]) {
    let cursor;
    do {
