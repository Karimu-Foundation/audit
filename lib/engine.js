...
/** The next stop on today's route that doesn't have a finished audit yet
 * (synced or not — once it's finished it's off the list either way).
 * Ordered stops first (in visiting order), then no-GPS stops. Pure local
 * lookup — everything it needs (route, group, past audits) already lives
 * on the device, so this works exactly the same with no connection at
 * all, which is the point: it's what tells the auditor where to send the
 * guide next right after finishing the audit they're on, well before any
 * sync happens back at the office. */
function nextRouteStop(){
  const volunteer = pref("volunteer");
  const current = currentRouteGroup(volunteer);
  if(!current) return null;
  const { ordered, unordered } = getRoute(volunteer, current.group);
  const notDone = (a) => !audits.some(x => x.type === "water_point" && x.assetTag === a.tag && x.completedAt);
  return ordered.find(notDone) || unordered.find(notDone) || null;
}
...
function screenSync(){
  ...
  const nextSub = next ? [next.village, next.sub].filter(Boolean).join(" · ") : "";
  ...
  ${next ? `
  <div class="card" style="padding:16px 18px;margin-bottom:16px;border-color:var(--accent)">
    <span class="eyebrow">${esc(ui("nextStopEyebrow"))}</span>
    <div class="audit-title" style="margin:5px 0 2px;font-size:17px">${esc(next.tag)}${nextSub ? " — " + esc(nextSub) : ""}</div>
    <div class="small" style="color:var(--muted);margin-bottom:14px">${esc(next.loc || "—")}</div>
    <button class="btn btn-primary btn-block" data-act="nextstop" data-tag="${esc(next.tag)}">${esc(ui("startNextStop"))}</button>
  </div>` : `
  <div class="banner banner-info" style="margin-bottom:16px">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
    <div>${esc(ui("routeAllDoneToday"))}</div>
  </div>`}
...
export function mount(){
  ...
  document.addEventListener("click", e => {
    ...
    else if(act === "nextstop"){ openOrStartAsset(t.dataset.tag); }
